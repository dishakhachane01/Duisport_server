...
class VendorAgingReport(models.TransientModel):
    _name = 'vendor.aging.report'
    _description = 'Vendor Aging Report'
    ...
    line_ids = fields.One2many('vendor.aging.line', 'report_id', string='Lines')
    ...
    def action_view_report(self):
        ...
        # Add partner filter if vendor is selected
        if self.partner_id:
        ...
        self.env['vendor.aging.line'].create(lines_to_create)
        
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'vendor.aging.report',
            'view_mode': 'form',
            'res_id': self.id,
            'target': 'new',
        }
    ...
    def action_download_xlsx(self):
        ...
        sheet = workbook.add_worksheet('Vendor Aging')
        
        # Headers
        headers = ['Vendor', 'Total Due', '0-30', '31-60', '61-90', '91-180', '181-365', '365+']
        ...
        attachment = self.env['ir.attachment'].create({
            'name': f'vendor_aging_{self.end_date}.xlsx',
            'type': 'binary',
    ...
    def action_download_pdf(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': '/report/pdf/all_reports_full.vendor_aging_pdf_template/%d' % self.id,
            'target': 'new',
        }

        # """Download PDF report"""
        # return self.env.ref('all_reports_full.report_vendor_aging_pdf').report_action(self)
...
class VendorAgingLine(models.TransientModel):
    _name = 'vendor.aging.line'
    _description = 'Vendor Aging Line'

    report_id = fields.Many2one('vendor.aging.report', string='Report', ondelete='cascade')
