...
class AccountMove(models.Model):
    ...
    def _compute_name(self):
        ...
        for move in self:
            if move.move_type == 'out_invoice' and move.name and move.name != '/':
                dt = move.invoice_date or dt_date.today()
                ...
                # Extract sequence number and apply 3-digit padding
                seq_number = move.name.split('/')[-1]
                seq_number = str(int(seq_number)).zfill(3)

                move.name = f"INV/{fy_start}-{str(fy_end)[-2:]}/{seq_number}"

    # def _get_sequence_format_param(self, previous):
    #     prefix, format_values = super()._get_sequence_format_param(previous)

    #     if self.move_type == 'out_invoice':
    #         dt = self.invoice_date or dt_date.today()
    ...
    #         prefix = f"INV/{fy_start}-{fy_end}/%(seq)s"

    #     return prefix, format_values
    ...
    #             # Replace prefix only, keep number
    #             parts = move.name.split('/')
    #             if parts:
    #                 number = parts[-1]
    #                 move.name = f"INV/{fy_start}-{fy_end}/{number}"